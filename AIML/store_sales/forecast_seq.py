...
print("[DATA SPLIT AND MANAGE] Start")
...
y_train_tensor = torch.tensor(y_train.values[sequence_length-1:], dtype=torch.float32).to(device)
X_valid_tensor = torch.tensor(X_valid_seq, dtype=torch.float32).to(device)
y_valid_tensor = torch.tensor(y_valid.values[sequence_length-1:], dtype=torch.float32).to(device)
...
print("[DATA SPLIT AND MANAGE] Finish")
